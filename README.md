...
### Класс `ShopApi` определен в файле `src/components/Api/ShopApi.ts`.

Расширяет базовый класс `Api` и реализует интерфейс `IApi`. Служит для взаимодействия с сервером интернет-магазина: загрузки товаров и оформления заказа.

Конструктор:
Наследуется от `Api`. При создании экземпляра принимает:  
`baseUrl: string` — базовый адрес сервера  
`options?: RequestInit` — объект с заголовками и настройками запросов  

Поля:  
`baseUrl: string` — базовый адрес сервера (унаследовано от `Api`)  
`options: RequestInit` — объект с заголовками запросов (унаследовано от `Api`)  

Методы:
`getProducts(): Promise<IProduct[]>` — выполняет запрос к серверу и возвращает список товаров  
`orderProducts(order: IOrder): Promise<IOrder>` — отправляет заказ на сервер и возвращает подтверждённый заказ  